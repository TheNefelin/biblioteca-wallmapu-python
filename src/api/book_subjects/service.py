...
# -----------------------------------------------------------------
# UPDATE (reemplaza relaciones; si subject_ids viene vacÃ­o, elimina todas)
async def update_subjects(db: AsyncSession, id_book: int, subject_ids: list[int]) -> list[BookSubjectResponse]:
  subject_ids = list(set(subject_ids or []))
  items = await repository.update(db, id_book, subject_ids)
  return [BookSubjectResponse.model_validate(item) for item in items]


# -----------------------------------------------------------------
# DELETE (elimina una relaciÃ³n book-subject)
...
# -----------------------------------------------------------------
# DELETE (elimina toda las relaciÃ³nes book-subject)
