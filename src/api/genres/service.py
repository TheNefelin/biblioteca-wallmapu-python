...
async def get_all_pagination(db: AsyncSession, pagination: PaginationRequestDTO) -> PaginationResponseDTO:
  ...
  data = [GenreResponse.model_validate(item) for item in items]
...
# -----------------------------------------------------------------#
# GET ALL
async def get_all(db: AsyncSession) -> list[GenreResponse]:
  items = await repository.get_all(db)
  return [GenreResponse.model_validate(item) for item in items]


# -----------------------------------------------------------------#
# CREATE
async def create(db: AsyncSession, dto: GenreRequest) -> GenreResponse:
...
# -----------------------------------------------------------------#
# UPDATE
async def update(db: AsyncSession, id: int, dto: GenreRequest) -> GenreResponse:
