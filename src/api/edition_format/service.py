...
async def update_formats(db: AsyncSession, id_edition: int, format_ids: list[int]) -> list[EditionFormatDTO]:
  format_ids = list(set(format_ids or []))
  items = await repository.update(db, id_edition, format_ids)
  return [EditionFormatDTO.model_validate(item) for item in items]
