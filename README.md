...
> Los tests ya están en uso (74 en verde: Books, Editions, Catálogo, Auth, Users, Loans, Reservations, Notifications). Requieren una BD aislada conforme a `TEST_DATABASE_URL` (ver `.env_demo`).
```sh
...
```
Definición: `tests/conftest.py` (fixtures `make_user`, `make_policy`) + tests por feature (`test_catalog_read`, `test_catalog_extended`, `test_catalog_additional`, `test_books`, `test_editions`, `test_auth`, `test_users`, `test_loans`, `test_reservations`, `test_notifications`). Base/seed: `postgre_base.sql` + `postgre_seed.sql`. Estado: **74 tests en verde**.
