...
> Los tests ya están en uso (82 en verde: Books, Editions, Catálogo, Auth, Users, Loans, Reservations, Notifications). Requieren una BD aislada conforme a `TEST_DATABASE_URL` (ver `.env_demo`).
```sh
...
### Tests
Los tests usan una BD aislada (`db_testing`) conforme a `TEST_DATABASE_URL` y restauran el esquema/seed en cada corrida (solo tablas `wm_*`). Ejecutar con `run_test.py` (o `pytest` sobre `tests/`):
```sh
.venv\Scripts\python.exe run_test.py
```
Definición: `tests/conftest.py` (fixtures `make_user`, `make_policy`) + tests por feature (`test_catalog_read`, `test_catalog_extended`, `test_catalog_additional`, `test_catalog_remaining`, `test_books`, `test_editions`, `test_auth`, `test_users`, `test_loans`, `test_reservations`, `test_notifications`). Base/seed: `postgre_base.sql` + `postgre_seed.sql`. Estado: **82 tests en verde**.
